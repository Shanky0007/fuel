...
const SOUTH_AFRICA_NAME = 'South Africa';
...
export default function RegisterScreen({ navigation }) {
    ...
    const fetchRegions = async () => {
        ...
        try {
            ...
            console.log('RegisterScreen regions:', data?.length);
            setRegions(Array.isArray(data) ? data : []);
        } catch (error) {
            console.error('Error fetching regions:', error?.message || error);
            setRegions([]);
        } finally {
    ...
    const handleRegister = async () => {
        ...
        if (!region) { showAlert('Error', 'Please select your region'); return; }
        ...
        try {
            await register(name, email, password, phone, SOUTH_AFRICA_NAME, region, city, vehicleType, fuelType, registrationNumber);
        } catch (error) {
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
                    ...
                    >
                        ...
                        <View style={styles.formCard}>
                            ...
                            <View style={[styles.input, styles.staticField]}>
                                <Text style={[styles.staticText]}>{SOUTH_AFRICA_NAME}</Text>
                            </View>
                            ...
                            <CustomPicker
                                ...
                                items={vehicleTypes.map(t => ({ label: t, value: t }))}
                            ...
                            <CustomPicker
                                ...
                                items={fuelTypes.map(t => ({ label: t, value: t }))}
...
const makeStyles = (colors) => StyleSheet.create({
    ...
    logoIcon: {
        width: 64, height: 64, backgroundColor: colors.amber,
    ...
    input: {
        backgroundColor: colors.bg3, paddingHorizontal: 16, borderRadius: 12,
        borderWidth: 1, borderColor: colors.border, fontSize: 15,
        color: colors.text, minHeight: 54, marginBottom: 16,
        justifyContent: 'center',
    },
    staticField: { opacity: 0.5 },
    staticText: { fontSize: 15, color: colors.text },
    button: {
